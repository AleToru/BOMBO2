...
Esta app SOLO se encarga de mostrar la interfaz. Todos los datos y la
lógica del modelo matemático viven en modelo.py.
"""
...
st.markdown(
    ...
    <style>
    .stMetric {
        background-color: rgba(135, 165, 200, 0.14);
        border: 1px solid rgba(135, 165, 200, 0.30);
        border-radius: 10px;
        padding: 12px 16px;
    }
...
# ---------------------------------------------------------------------------
# Datos (embebidos en modelo.py, no se leen archivos externos)
# ---------------------------------------------------------------------------
...
tab_datos, tab_modelo, tab_resultado, tab_escenarios, tab_preguntas = st.tabs(
    ["📊 1. Datos", "🧮 2. Modelo matemático", "✅ 3. Solución óptima", "🔄 4. Escenarios", "📝 5. Preguntas del laboratorio"]
)
...
with tab_datos:
    ...
    etiquetas = [str(p) for p in puntos]
    max_dist = max(max(row) for row in matriz_base)
    ...
    fig_heatmap = go.Figure(
        data=go.Heatmap(
            ...
            y=etiquetas,
            colorscale="Blues",
            # Se limita el tope de la escala de color (en vez de usar el máximo real)
            # para que ninguna celda quede tan oscura que el texto se vuelva ilegible.
            zmin=0,
            zmax=max_dist * 1.6,
            ...
            xgap=2,
            ygap=2,
            hovertemplate="Desde %{y} hasta %{x}: %{z:.0f} km<extra></extra>",
    ...
    fig_heatmap.update_layout(
        ...
        xaxis=dict(type="category", tickmode="array", tickvals=etiquetas, title="Hasta", side="top"),
        yaxis=dict(type="category", tickmode="array", tickvals=etiquetas, autorange="reversed", title="Desde"),
    ...
    with st.expander("Ver matriz como tabla de números"):
...
necesita que la matriz sea simétrica y no negativa.
    ...
    fig_mapa.add_trace(
        go.Scatter(
            ...
            textfont=dict(color="#0E1117", size=13),
    ...
    if st.button("▶️ Ejecutar optimización", type="primary"):
        with st.spinner("Resolviendo el modelo (puede tardar unos segundos)..."):
            resultado: ResultadoTSP = resolver_cacheado(
                puntos, tuple(tuple(row) for row in matriz_base), DEPOSITO
            )
    ...
    if st.button("🔁 Recalcular con el cambio"):
        ...
        try:
            ...
            with st.spinner("Resolviendo escenario modificado..."):
                resultado_mod = resolver_tsp(puntos, matriz_mod, DEPOSITO)
...
# ---------------------------------------------------------------------------
# TAB 5 — Preguntas del laboratorio (guía de referencia, no respuestas)
# ---------------------------------------------------------------------------
with tab_preguntas:
    st.subheader("Las 11 preguntas del laboratorio (Bloque 3 · Caso 2)")
    st.caption(
        "Esta pestaña es solo una guía de referencia con el enunciado de cada pregunta "
        "y dónde encontrar en esta app la información para responderla. Las respuestas "
        "(máximo 5 líneas cada una) y la declaración jurada se entregan por el enlace "
        "de Formspree, como indica la clase."
    )

    st.markdown("### Parte I · Comprensión del enfoque — 30 pts")
    st.markdown(
        """
1. **(10 pts)** ¿Por qué no se resuelve el TSP directamente sobre toda la red vial original,
   sino sobre una matriz de distancias reducida? → *Ver pestaña 1, introducción.*
2. **(10 pts)** ¿Qué representa una solución del TSP en el contexto de esta empresa de
   distribución? → *Ver pestaña 3, secuencia de visita.*
3. **(10 pts)** ¿Por qué esta instancia (13 puntos) corre en cualquier versión de AMPL, aun
   sin licencia comercial? Reporte el número de variables y restricciones, y compárelo con
   una red vial completa. → *Ver pestaña 2, "Tamaño del modelo".*
        """
    )

    st.markdown("### Parte II · Implementación en AMPL — 45 pts")
    st.markdown(
        """
4. **(10 pts)** Formule la función objetivo del TSP. ¿Qué minimiza? → *Ver pestaña 2,
   "Función objetivo".*
5. **(10 pts)** Escriba las restricciones de salida única y entrada única de cada punto.
   → *Ver pestaña 2, R1 y R2.*
6. **(15 pts)** Escriba las restricciones de eliminación de subciclos y explique por qué son
   necesarias. → *Ver pestaña 2, R3 (MTZ) y el expander "¿Por qué R1+R2 no bastan?".*
7. **(10 pts)** Ejecute el modelo con los 13 puntos (depósito + 12 clientes). Reporte la
   distancia total, los arcos seleccionados y la secuencia de visita. → *Ver pestaña 3,
   botón "Ejecutar optimización".*
        """
    )

    st.markdown("### Parte III · Análisis y escenarios — 25 pts")
    st.markdown(
        """
8. **(10 pts)** Si se agrega un cliente nuevo, ¿cómo cambia el tamaño del modelo (variables
   y restricciones) y por qué ese crecimiento importa? → *Ver pestaña 4, "Simular un cliente
   nuevo".*
9. **(5 pts)** Si una distancia aumenta por el cierre temporal de una carretera, ¿cómo puede
   afectar la ruta óptima? → *Ver pestaña 4, "Simular cierre temporal de una vía".*
10. **(10 pts)** ¿Qué información adicional habría que guardar para reconstruir la ruta vial
    completa entre clientes, no solo la distancia total? → *Para responder: piense en qué se
    perdió al "comprimir" la red vial completa en una sola distancia por par de puntos (la
    matriz solo guarda el número, no el camino físico que lo genera).*
        """
    )
    ...
    st.markdown(
        """
<div class="nota-caja">
<b>Recordatorio de logística:</b> trabajo en equipo, explicaciones de máximo 5 líneas,
declaración jurada de trabajo propio, tiempo estimado 150 minutos. Entregar el archivo de
respuestas y el archivo de la herramienta (este modelo y su corrida) por el enlace de
Formspree.
</div>
        """,
        unsafe_allow_html=True,
...
st.markdown("---")
st.caption(
    "Caso 2 · Modelo TSP con formulación MTZ · Datos embebidos en modelo.py · "
